...
export const inboxContactQuerySchema = z
  .object({
    site_id: uuidSchema,
...
export async function GET(req: Request) {
  try {
    ...
    const rateLimit = await enforceRateLimit(
      ctx.admin,
      rateLimitAuthKey(
        hasSearch ? "inbox_contact_search" : "inbox_contact_list",
        ctx.tenant.id,
        ctx.user.id,
      ),
    ...
    let dataQuery = ctx.supabase
      ...
      .select(
        "id,tenant_id,site_id,full_name,email,phone,subject,message,kvkk_accepted_at,source,is_read,created_at",
      )
