...
export const inboxApplicationsQuerySchema = z
  .object({
    site_id: uuidSchema,
...
export async function GET(req: Request) {
  try {
    ...
    const rateLimit = await enforceRateLimit(
      ctx.admin,
      rateLimitAuthKey(
        hasSearch ? "inbox_applications_search" : "inbox_applications_list",
        ctx.tenant.id,
        ctx.user.id,
      ),
    ...
    let dataQuery = ctx.supabase
      ...
      .select(
        "id,tenant_id,site_id,job_post_id,full_name,email,phone,message,cv_path,kvkk_accepted_at,is_read,created_at,job_post:job_posts(id,title)",
      )
    ...
    if (parsed.data.job_post_id) dataQuery = dataQuery.eq("job_post_id", parsed.data.job_post_id);
    ...
    if (parsed.data.search) {
      dataQuery = dataQuery.or(buildSafeIlikeOr(["full_name", "email"], parsed.data.search));
    }
    ...
    const total = await getOrSetCachedValue<number>(countCacheKey, INBOX_COUNT_CACHE_TTL_SEC, async () => {
      ...
      if (parsed.data.job_post_id) countQuery = countQuery.eq("job_post_id", parsed.data.job_post_id);
      ...
      if (parsed.data.search) {
        countQuery = countQuery.or(buildSafeIlikeOr(["full_name", "email"], parsed.data.search));
      }
