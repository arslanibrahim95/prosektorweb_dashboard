...
const PLATFORM_ANALYTICS_CACHE_TTL_SEC = 120;
...
export const GET = withAdminErrorHandling(async (req: Request) => {
    ...
    const cacheKey = ["platform-analytics", ctx.user.id].join("|");

    const payload = await getOrSetCachedValue(cacheKey, PLATFORM_ANALYTICS_CACHE_TTL_SEC, async () => {
      ...
      return platformAnalyticsResponseSchema.parse({
        totals: {
