...
import {
  appointmentModuleSettingsSchema,
...
export default function AppointmentModulePage() {
  const site = useSite();
  const siteId = site.currentSiteId;

  const [newEmail, setNewEmail] = useState('');
  const [draftBySite, setDraftBySite] = useState<Record<string, Partial<AppointmentFormState>>>({});
  ...
  const serverForm = useMemo<AppointmentFormState>(() => {
    const appointmentModule = modules?.find((module) => module.module_key === 'appointment');
    if (!appointmentModule) {
      return EMPTY_APPOINTMENT_FORM;
    }

    const parsed = appointmentModuleSettingsSchema.safeParse(appointmentModule.settings ?? {});
    if (!parsed.success) {
      return {
        ...EMPTY_APPOINTMENT_FORM,
        isEnabled: Boolean(appointmentModule.enabled),
      };
    }
  ...
  const formState = useMemo<AppointmentFormState>(() => {
    return {
  ...
  const updateDraft = useCallback(<K extends keyof AppointmentFormState>(key: K, value: AppointmentFormState[K]) => {
    if (!siteId) return;
  ...
  return (
    <div className={cn('dashboard-page', 'dashboard-page-narrow', 'stagger-children')}>
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-foreground">Randevu Modülü</h1>
        <p className="text-muted-foreground mt-1">Randevu talebi formu ayarlarını yapılandırın</p>
      </div>
      ...
      <Card className="glass border-border/50 shadow-sm overflow-hidden">
        <CardHeader className="pb-4">
          ...
          <CardDescription>Randevu formunu etkinleştirin veya devre dışı bırakın</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-between">
            <div>
              <Label className="font-medium">Randevu Formu Aktif</Label>
      ...
      <Card className="glass border-border/50 shadow-sm">
        <CardHeader className="pb-4">
          ...
          <CardDescription>Yeni randevu talebi geldiğinde bildirim alacak email adresleri</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-2">
      ...
      <div className="flex justify-end gap-2 pb-4">
        ...
        <ActionButton
          ...
          successLabel="Randevu modülü ayarları kaydedildi!"
